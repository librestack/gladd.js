...
function getTabByTitle(title) {
    var tabid = $('li.tabhead.business' + g_business)
        .find('a.tabtitle:contains("' + title + '")')
        .attr("href");
        ...
        for (menu in g_menus) {
            if ($(this).attr("href") == '#' + g_menus[menu][0]) {
                if (g_menus[menu][1] == alert) {
                    ...
                    g_menus[menu][1].apply(this,
                            Array.prototype.slice.call(g_menus[menu], 2));
                }
                ok = true;
                break;
            }
...
function fixXMLDates(xml) {
    $(xml).find('transactdate').each(function() {
        ...
        if (mydate.isValid()) {
            $(this).replaceWith('<' + this.tagName + '>'
                + mydate.format('YYYY-MM-DD') + '</' + this.tagName + '>');
        }
    });
...
function submitForm(object, action, id) {
    ...
    mytab.find('div.' + object)
    .find('form:not(.subform)')
    .each(function() {
...
Form.prototype.show = function(tab) {
    ...
    if (tab === undefined && this.tab === undefined) {
        ...
        this.tab = tab;
        tab.form = this; /* link form and tab to each other */
    }
